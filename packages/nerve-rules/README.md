...
22 built-in validation rules with stable `HK-*` codes: missing ground returns, untwisted differential pairs (CAN/RS-485/USB, bus-indexed), ampacity vs. gauge, terminal/seal compatibility, reserved pins, bend radius, bundle-vs-sleeve capacity, voltage ratings, and more — each one a field failure caught at compile time. Includes a derived numeric code mapping for tooling.
